...
def part1_countinvalidtickets(ticket_data):	
	...
	return sum(value for ticket in ticket_data['nearyby_tickets'] for value in ticket if not _checkinrange(value, field_ranges))
...
def part2_productofdepartures(ticket_data):
	...
	valid_tickets = [ticket for ticket in ticket_data['nearyby_tickets'] if all(_checkinrange(value, field_ranges) for value in ticket)]
	...
	return reduce(operator.mul, [ticket_data['your_ticket'][pos] for field,pos in field_positions.items() if field.startswith('departure')])
		...
		for line in file:
			if line.strip():
				...
				if field != 'your ticket' and field != 'nearby tickets' and not nearyby_tickets and not your_ticket:
					...
					ticket_data['fields'][field] = tuple(tuple(map(int,x.strip().split('-'))) for x in data.strip().split(' or '))
...
if __name__ == '__main__':
	...
	ticket_data = read_input("day16_input.txt")
